class Api::V1::OrdersController < ApplicationController
  ...
  def checkout
    ...
    if order.save
      ...
      cart.cart_items.each do |cart_item|
        order_item = order.order_items.build(product_id: cart_item.product_id, quantity: cart_item.quantity, cart_id: cart_item.cart_id)
        unless order_item.save
          # Handle errors during order_item creation
          render json: { message: 'There was an error creating the order.', errors: order_item.errors.full_messages }, status: :unprocessable_entity
          return
        end
      end
      ...
      render json: { message: 'There was an error creating the order.', errors: order.errors.full_messages }, status: :unprocessable_entity
    end
  end
  ...
  def order_params
    params.require(:order).permit(:order_date, :total_amount, :status, :payment_method, :cart_id, shipping_address: [:street, :city, :state, :zip_code])
  end
